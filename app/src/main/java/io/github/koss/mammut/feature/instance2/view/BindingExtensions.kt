...
fun InstanceFragmentTwoBinding.showFeedTypeButton(animate: Boolean = true) {
    if (animate) {
        feedTypeButton.animate()
                .translationY(0F)
                .setInterpolator(OvershootInterpolator())
                .setDuration(300L)
                .start()
    } else {
        feedTypeButton.translationY = 0F
    }
}

fun InstanceFragmentTwoBinding.hideFeedTypeButton(animate: Boolean = true) {
    if (animate) {
        feedTypeButton.animate()
                .translationY(-(feedTypeButton.y + feedTypeButton.height))
                .setInterpolator(AccelerateInterpolator())
                .setDuration(150L)
                .start()
    } else {
        feedTypeButton.translationY = -(feedTypeButton.y + feedTypeButton.height)
    }
}